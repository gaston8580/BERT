...
def main(args):
    ...
    # 定义dataset(torch)，加载数据集
    train_dataset = load_and_cache_examples(args, tokenizer, mode="train")  # TensorDataset数据类型
    dev_dataset = load_and_cache_examples(args, tokenizer, mode="dev")  # TensorDataset数据类型
    test_dataset = load_and_cache_examples(args, tokenizer, mode="test")  # TensorDataset数据类型

    # 加载trainer
    trainer = Trainer(args, train_dataset, dev_dataset, test_dataset)  # dataloder在里面
...
if __name__ == '__main__':
    ...
    parser.add_argument("--task", default="sentiment", required=False, type=str, help="The name of the task to train")
    parser.add_argument("--model_dir", default="experiments/outputs/clsbert_1", required=False, type=str, help="Path to save, load model")
